...
The Archer D1X seems as though it may be suseptible to replay attacks, however we haven't been able finish testing this. During setup, only the reciever near the derailleur produces a bluetooth pairing packet, and in our case it had a MAC address of 00:A0:50:89:46:CA, which is non-standard. Particularly of note devices aren't supposed to start with "00", and this created a number of trivial problems while trying to listen to packets and replay them (as some software will tell you this is an illegal address instead of listening/sending to the address).

Using the Ubertooth we were able to listen to and record packets, and as advertised these packets were enrypted, both to the device from a phone and between the shifter and reciever near the derailleur.

Additionally of interest the software isn't updatable over the internet, preventing spoofing the firmware remotely.

\subsection{Shimano Replay Attacks}

Using an Ubertooth One we were able to catch pairing packets (which were given the device name EWWU111) however we weren't able to successfully replay packets and make the gear shift. When viewing them, all the packets were encrypted as well, even when we had no passcode.

We also attempted to break the 6-pin passcode using a tool called Crackle. Crackle exploits a design flaw in bluetooth pairing process which is supposed to allow an attacker to brute force a temporary key relativly quickly using the packets found while listening to a legitamate transmission. To do this, we saved .pcap files of the pairing packets and attempted to run them through crackle, however we think the Shimano Di2 system classifies Mrand and Srand values irregularly, as after repeated attempts we weren't able to crack it.

DON'T PUBLISH THIS PARAGRAPH AS FINAL DRAFT, DOS HASN'T WORKED YET, THESE ARE MY ASSUMPTIONS Lastly we attempted to confuse the device using a DoS attack. Although all wireless devices are suseptible to DoS, one thought we had was that if we flood the wireless unit with ping requests, it might either flood the CAN bus network or prevent other CAN bus packets from passing between the wireless unit and other units (such as the shifter and derailure).. To do this we ran blue doser against the unit, and tried moving the wireless unit to two different positions on the Di2 network. In both cases we weren't able to prevent manual shifts from shifter, however it did disconnect our phones as expected.
